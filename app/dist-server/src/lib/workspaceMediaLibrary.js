...
export const formatWorkspaceMediaLibraryCreatedAt = (value, locale = "ru") => {
...
const getPositiveWorkspaceMediaLibraryAssetId = (value) => typeof value === "number" && Number.isFinite(value) && value > 0 ? Math.trunc(value) : null;
export const isWorkspaceMediaLibraryStableAssetUrl = (value, assetId) => {
    const normalizedValue = String(value ?? "").trim();
    if (!normalizedValue) {
        return false;
    }
...
export const isWorkspaceMediaLibraryTransientStudioJobUrl = (value) => {
    const normalizedValue = String(value ?? "").trim();
    if (!normalizedValue) {
        return false;
    }
...
export const isWorkspaceMediaLibraryLegacyFallbackDownloadUrl = (value) => {
    const normalizedValue = String(value ?? "").trim();
    if (!normalizedValue) {
        return false;
    }
...
const getWorkspaceMediaLibrarySourceRank = (item) => {
...
const getWorkspaceMediaLibraryItemUrls = (item) => [item.previewUrl, item.previewPosterUrl, item.downloadUrl];
export const hasWorkspaceMediaLibraryStableAssetUrl = (item) => {
...
export const hasWorkspaceMediaLibraryTransientStudioJobUrl = (item) => getWorkspaceMediaLibraryItemUrls(item).some(isWorkspaceMediaLibraryTransientStudioJobUrl);
export const hasWorkspaceMediaLibraryLegacyFallbackDownloadUrl = (item) => isWorkspaceMediaLibraryLegacyFallbackDownloadUrl(item.downloadUrl);
export const isWorkspaceMediaLibraryItemDurableForStorage = (item) => {
...
const shouldPreferWorkspaceMediaLibraryAssetDuplicate = (candidate, existing) => {
    ...
    const specificityDifference = getWorkspaceMediaLibraryItemSpecificityRank(candidate) -
        getWorkspaceMediaLibraryItemSpecificityRank(existing);
    if (specificityDifference !== 0) {
        return specificityDifference > 0;
    }
    const stableRouteDifference = Number(hasWorkspaceMediaLibraryStableAssetUrl(candidate)) -
...
export const dedupeWorkspaceMediaLibraryPageItems = (items) => {
    ...
    const itemIndexesByPrimaryKey = new Map();
    const itemIndexesByVideoModeSlot = new Map();
    const itemIndexesByAssetId = new Map();
    const result = [];
    const indexKeys = new Map();
    const unregisterIndex = (index) => {
    ...
    const registerItemAtIndex = (index, item) => {
        const primaryKey = getWorkspaceMediaLibraryResolvedDedupeKey(item);
        const videoModeSlotKey = getWorkspaceMediaLibraryVideoModeSlotKey(item);
        const assetId = getPositiveWorkspaceMediaLibraryAssetId(item.assetId);
    ...
    for (const item of sortedItems) {
        ...
        if (existingVideoModeSlotIndex === existingIndex &&
