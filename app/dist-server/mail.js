...
const createTransport = async () => {
    if (authProviderStatus.smtpConfigured) {
        ...
        if (env.smtpHost?.toLowerCase().includes("gmail") && env.smtpUser && !env.smtpFrom.includes(env.smtpUser)) {
            console.warn("[mail] Gmail SMTP: адрес в SMTP_FROM должен совпадать с SMTP_USER (или быть добавленным алиасом в настройках Gmail), иначе письма часто не доставляются.");
        }
        try {
            ...
            console.error("[mail] SMTP verify failed — письма, скорее всего, не отправятся. Проверьте SMTP_*, для Gmail нужен пароль приложения; если в пароле есть пробелы, задайте SMTP_PASS в кавычках в .env.", error);
        }
        return {
            from: env.smtpFrom,
            mode: "smtp",
