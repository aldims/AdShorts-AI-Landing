...
export const resolveWorkspaceSubscriptionDetailsFromAdminPayload = (payload, options) => {
    const successfulPayments = Array.isArray(payload.payments)
        ...
        : [];
    const hasSuccessfulStartPayment = successfulPayments.some((payment) => String(payment?.plan_code ?? "").trim().toLowerCase() === "start");
    ...
    const resolvePlanLabel = (planCode) => planCode ? planCode.toUpperCase() : null;
...
const normalizeStudioSegmentVisualQuality = (value) => {
    const normalized = String(value ?? "").trim().toLowerCase();
...
const WORKSPACE_CHARACTER_REFERENCE_GPT_IMAGE_2_QUALITY = "medium";
const WORKSPACE_CHARACTER_REFERENCE_GPT_IMAGE_2_RESOLUTION = "2k";
const studioWaveSpeedSegmentAiVideoJobContexts = new Map();
const studioWaveSpeedSegmentAiPhotoJobContexts = new Map();
const studioDirectSegmentAiPhotoJobContexts = new Map();
...
const isDirectSegmentAiPhotoJobId = (jobId) => normalizeGenerationText(jobId).startsWith(DIRECT_SEGMENT_AI_PHOTO_JOB_PREFIX);
...
const normalizeDirectSegmentAiPhotoFileName = (jobId, referenceKind, mimeType) => {
    const normalizedKind = normalizeGenerationText(referenceKind).replace(/[^a-z0-9_-]+/gi, "-") || "reference";
...
const buildWorkspaceReferenceCharacterSheetPrompt = (prompt, options) => {
    const normalizedPrompt = normalizePrompt(prompt);
    const sourceInstruction = options?.sourceMode === "reference_image"
...
export const normalizeStudioVoiceIdForLanguage = (voiceId, language) => {
    const normalizedVoiceId = normalizeGenerationText(voiceId);
    if (!normalizedVoiceId || normalizedVoiceId === "none") {
        return undefined;
    }
...
const normalizeAdsflowVoiceSourceWindow = (payload) => {
    ...
    const voiceSourceDuration = explicitDuration !== null && explicitDuration > 0
    ...
    const voiceSourceEndTime = sourceEndTime !== null
...
export const buildStudioSegmentVisualDurationPayload = (durationSeconds) => {
...
export const normalizeStudioSegmentEditorPayload = (value, language, fallbackProjectId, options) => {
    ...
    rawSegments.forEach((segment) => {
        ...
        const manualDurationSeconds = rawManualDurationSeconds ??
...
const buildStudioProjectVoiceoverJobAudioProxyUrl = (jobId) => {
    const normalizedJobId = normalizeGenerationText(jobId);
    if (!normalizedJobId) {
        return null;
    }
    const proxyUrl = new URL(`/api/studio/project-voiceover/jobs/${encodeURIComponent(normalizedJobId)}/audio`, env.appUrl);
    return `${proxyUrl.pathname}${proxyUrl.search}`;
};
...
const hasStudioFinalVideoDownload = (value) => Boolean(normalizeGenerationText(value));
export const canExposeStudioFinalVideoFromStatus = (options) => {
    ...
    return (normalizedReadyReason === "project_not_ready" ||
...
const getStudioGenerationPublicStatus = (options) => (canExposeStudioFinalVideoFromStatus(options) ? "done" : String(options.status ?? "queued"));
...
const requestStudioVisualPromptEnglishTranslation = async (prompt, sourceLanguage, model, options) => {
    const response = await fetch(`${env.openrouterBaseUrl.replace(/\/+$/, "")}/chat/completions`, {
        method: "POST",
        headers: {
            Accept: "application/json",
...
const translateStudioGenerationPromptToEnglish = async (prompt, options) => {
    ...
    if (env.openrouterApiKey && modelCandidates.length > 0) {
        for (const model of modelCandidates) {
            const remainingTimeoutMs = typeof timeoutBudgetMs === "number" && timeoutStartedAt !== null
...
const fetchStudioMediaAssetImage = async (assetId, externalUserId) => {
    const normalizedAssetId = normalizePositiveInteger(assetId);
    if (!normalizedAssetId) {
        throw new Error("Source media asset id is required.");
    }
    const response = await fetch(buildAdsflowUrl(`/api/media/${normalizedAssetId}/download`, {
    ...
    const arrayBuffer = await response.arrayBuffer();
    const mimeType = inferStudioGeneratedImageMimeType(response.headers.get("content-type"), null, `/api/media/${normalizedAssetId}/download`);
...
const normalizeWaveSpeedSegmentAiVideoAsset = (jobId) => {
    const remoteUrl = buildStudioSegmentAiVideoJobVideoProxyUrl(jobId);
    if (!remoteUrl) {
        throw new Error("Generated video is unavailable.");
    }
...
const normalizeWaveSpeedSegmentPhotoAnimationAsset = (jobId) => {
...
const normalizeAdsflowBatchVoiceoverSegmentAsset = (jobId, fallbackFileName, payload) => {
    const remoteUrl = buildStudioBatchVoiceoverSegmentAudioUrl(jobId, payload);
    if (!remoteUrl) {
        throw new Error("Generated batch voiceover segment is unavailable.");
    }
    return {
        assetId: normalizePositiveInteger(payload?.media_asset_id) ?? null,
        fileName: normalizeGenerationText(payload?.file_name) || fallbackFileName,
        fileSize: Math.max(0, Number(payload?.file_size ?? 0)),
        mimeType: normalizeGenerationText(payload?.mime_type) || "audio/wav",
        remoteUrl,
    };
};
const normalizeAdsflowProjectVoiceoverAsset = (jobId, payload) => {
    const remoteUrl = buildStudioProjectVoiceoverJobAudioProxyUrl(jobId);
    if (!remoteUrl) {
        throw new Error("Generated project voiceover is unavailable.");
    }
    return {
        assetId: normalizePositiveInteger(payload?.media_asset_id) ?? null,
        fileName: normalizeGenerationText(payload?.file_name) || `project-voiceover-${jobId}.wav`,
        fileSize: Math.max(0, Number(payload?.file_size ?? 0)),
        mimeType: normalizeGenerationText(payload?.mime_type) || "audio/wav",
        remoteUrl,
    };
...
const fetchAdsflowSubscriptionDetailsForBootstrap = async (externalUserId, user) => {
    ...
    const payload = await fetchAdsflowJson(buildAdsflowUrl("/api/admin/users", {
        page_size: "5",
        q: email,
    }), {
        headers: {
...
const restoreAdsflowStartSubscriptionAfterBootstrap = async (details, bootstrapProfile) => {
    if (details?.plan !== "START" || !details.userId || bootstrapProfile.plan === "START") {
        return;
    }
    try {
        await fetchAdsflowJson(buildAdsflowUrl(`/api/admin/users/${encodeURIComponent(details.userId)}/change-subscription`, {
            days: "30",
...
const fetchAdsflowSubscriptionDetailsForWebMutation = async (externalUserId, user) => fetchAdsflowSubscriptionDetailsForBootstrap(externalUserId, user).catch((error) => {
    console.warn("[studio] Failed to load pre-mutation AdsFlow subscription details", {
        error: error instanceof Error ? error.message : "Unknown error.",
    });
    return null;
});
const enrichWorkspaceProfileAfterAdsflowWebMutation = async (payload, rawUserId, subscriptionDetails) => {
...
const isStudioGenerationStatusDeleted = (status, deletedProjects) => {
    ...
    const adId = typeof generation?.adId === "number" && Number.isFinite(generation.adId) && generation.adId > 0
...
const removeDeletedStudioGenerationStatus = (status, deletedProjects) => (isStudioGenerationStatusDeleted(status, deletedProjects) ? null : status);
...
const findWorkspaceHistoryFallbackGeneration = async (user, excludedVideoUrls = [], deletedProjects = []) => {
...
const getAdsflowHealthComponentStatus = (value) => {
    if (!value || typeof value !== "object") {
...
const isAdsflowHealthComponentHealthy = (value) => {
    const status = getAdsflowHealthComponentStatus(value);
    return status === "healthy" || status === "ok";
};
const getAdsflowHealthWorkersOnlineCount = (value) => {
    if (!value || typeof value !== "object") {
        return null;
    }
    const workerPayload = value;
...
export async function ensureStudioGenerationWorkersAvailable() {
...
const fetchAdsflowProjectVoiceoverJobStatus = async (jobId, user) => {
    assertAdsflowConfigured();
    const safeJobId = String(jobId ?? "").trim();
    if (!safeJobId) {
        throw new Error("Job id is required.");
    ...
    const externalUserId = await resolveStudioExternalUserId(user);
    return fetchAdsflowJson(buildAdsflowUrl(`/api/web/project-voiceover/jobs/${encodeURIComponent(safeJobId)}`, {
        admin_token: env.adsflowAdminToken ?? "",
        external_user_id: externalUserId,
    }));
};
const fetchAdsflowBatchVoiceoverJobStatus = async (jobId, user) => {
    assertAdsflowConfigured();
    const safeJobId = String(jobId ?? "").trim();
    if (!safeJobId) {
        throw new Error("Job id is required.");
    ...
    const externalUserId = await resolveStudioExternalUserId(user);
    return fetchAdsflowJson(buildAdsflowUrl(`/api/web/voiceover/batch-jobs/${encodeURIComponent(safeJobId)}`, {
        admin_token: env.adsflowAdminToken ?? "",
        external_user_id: externalUserId,
    }));
};
...
export async function getWorkspaceBootstrap(user, options = {}) {
    ...
    const preBootstrapSubscriptionDetails = await fetchAdsflowSubscriptionDetailsForBootstrap(externalUserId, user).catch((error) => {
    ...
    try {
        ...
        let latestGeneration = removeDeletedStudioGenerationStatus(await prepareStudioLatestGenerationForBootstrap(buildLatestGenerationStatus(payload.latest_generation, latestHistoryEntry), user), deletedProjects);
        ...
        latestGeneration = removeDeletedStudioGenerationStatus(await prepareStudioLatestGenerationForBootstrap(latestGeneration, user), deletedProjects);
        if (latestGeneration?.generation) {
            warmStudioGenerationPlayback(latestGeneration.generation, user);
        }
        if (!cachedBootstrap) {
...
export async function createStudioGenerationJob(prompt, user, options) {
    ...
    const hasCustomVideoSource = normalizedVideoMode === "custom" &&
        Boolean(normalizedCustomVideoAssetId ||
        ...
        const normalizedSegmentEditorAssetPayload = normalizedSegmentEditor
            ? {
                ...
                segments: await Promise.all(normalizedSegmentEditor.segments.map(async (segment) => {
                    ...
                    const uploadScopeSegmentIndex = normalizedSegmentEditor.allowStructureChange && !isTalkingPhotoSegment ? undefined : segment.index;
...
export async function createStudioSegmentAiPhotoJob(prompt, user, options) {
        ...
        try {
            ...
            if (!jobCreated) {
                try {
                    ...
                    jobCreated = true;
                    console.warn("[studio] WaveSpeed GPT Image 2 text-to-image failed; generated workspace reference via DEAPI fallback", error);
                    return directJob;
                }
                catch (fallbackError) {
                    const waveSpeedMessage = error instanceof Error ? error.message : "WaveSpeed GPT Image 2 request failed.";
                    const fallbackMessage = fallbackError instanceof Error ? fallbackError.message : "DEAPI image fallback failed.";
                    error = new Error(`Не удалось сгенерировать референс. WaveSpeed: ${waveSpeedMessage}. DEAPI: ${fallbackMessage}.`);
                }
                try {
                    await refundWorkspaceGenerationCredit(user, creditReservation.consumed, normalizedLanguage);
                }
    ...
    const payload = await postAdsflowJson("/api/web/segment-ai-photo/jobs", {
        ...
        library_kind: normalizedPurpose === "workspace_reference" && normalizedReferenceKind
...
export async function createStudioSegmentSceneSoundJob(prompt, user, options) {
    ...
    const visualSourceKind = normalizedVisualSourceKind === "segment-ai-video" ||
...
export async function createStudioProjectVoiceoverJob(text, user, options) {
    assertAdsflowConfigured();
    ...
    const segments = (options?.segments ?? [])
        ...
        return {
            duration: normalizeNumber(segment.targetDurationSeconds),
            segment_index: normalizeNonNegativeInteger(segment.segmentIndex) ?? index,
            target_duration: normalizeNumber(segment.targetDurationSeconds),
            text: segmentText,
        };
    })
        .filter((segment) => Boolean(segment));
    if (!normalizedProjectId) {
        throw new Error("Project id is required for project voiceover generation.");
    }
    if (!normalizedVoiceType || normalizedVoiceType === "none") {
        throw new Error("Voice type is required for project voiceover generation.");
    }
    if (segments.length === 0) {
        throw new Error("At least one voiceover segment is required.");
    }
    const externalUserId = await resolveStudioExternalUserId(user);
    const subscriptionDetails = await fetchAdsflowSubscriptionDetailsForWebMutation(externalUserId, user);
    let payload;
    try {
        payload = await postAdsflowJson("/api/web/project-voiceover/jobs", {
            admin_token: env.adsflowAdminToken,
            credit_cost: requiredCredits,
            external_user_id: externalUserId,
            language: normalizedLanguage,
        ...
        if (isAdsflowHttpStatusError(error, 404)) {
            throw new Error("AdsFlow project voiceover endpoint is not deployed. Deploy /api/web/project-voiceover/jobs before enabling whole-video voiceover generation.");
        }
        throw error;
    }
    const jobId = String(payload.job_id ?? "").trim();
...
const studioBatchVoiceoverFallbackJobs = new Map();
const studioBatchVoiceoverJobMetadata = new Map();
const STUDIO_BATCH_VOICEOVER_FALLBACK_JOB_TTL_MS = 45 * 60 * 1000;
const cleanupStudioBatchVoiceoverFallbackJobs = () => {
    const now = Date.now();
    for (const [jobId, job] of studioBatchVoiceoverFallbackJobs) {
        if (now - job.createdAt > STUDIO_BATCH_VOICEOVER_FALLBACK_JOB_TTL_MS) {
            studioBatchVoiceoverFallbackJobs.delete(jobId);
        }
    }
    for (const [jobId, metadata] of studioBatchVoiceoverJobMetadata) {
        if (now - metadata.createdAt > STUDIO_BATCH_VOICEOVER_FALLBACK_JOB_TTL_MS) {
            studioBatchVoiceoverJobMetadata.delete(jobId);
        }
    }
};
const isStudioVoiceoverReadyStatus = (value) => {
    const status = String(value ?? "").trim().toLowerCase();
    return ["completed", "done", "ready", "success", "succeeded"].includes(status);
};
const isStudioVoiceoverFailedStatus = (value) => {
    const status = String(value ?? "").trim().toLowerCase();
...
const normalizeStudioBatchVoiceoverGroups = (groups) => {
    const normalizedGroups = (groups ?? [])
        .map((group) => {
        const language = normalizeStudioLanguage(group.language);
        const voiceType = normalizeStudioVoiceIdForLanguage(group.voiceType, language);
        ...
        const segments = (group.segments ?? [])
            ...
            return {
                segmentIndex: normalizeNonNegativeInteger(segment.segmentIndex) ?? index,
                targetDurationSeconds: normalizeNumber(segment.targetDurationSeconds),
                text,
            };
        })
            .filter((segment) => Boolean(segment));
    ...
    normalizedGroups.forEach((group) => {
        const key = `${group.language}:${group.voiceType.toLowerCase()}`;
        const existing = mergedGroups.get(key);
        if (existing) {
            existing.segments.push(...group.segments);
            return;
        }
...
const normalizeAdsflowBatchVoiceoverStatusSegment = (payload, fallback) => {
    const segmentIndex = normalizeNonNegativeInteger(payload.segmentIndex ?? payload.segment_index) ??
        ...
        fallback.index;
    const language = normalizeStudioLanguage(payload.language ?? fallback.language);
    const voiceType = normalizeStudioVoiceIdForLanguage(payload.voiceType ?? payload.voice_type ?? fallback.voiceType, language);
    if (!voiceType || voiceType === "none") {
        throw new Error("Batch voiceover segment voice type is missing.");
    }
    const jobId = normalizeGenerationText(payload.job_id);
    const asset = payload.asset
        ? normalizeAdsflowBatchVoiceoverSegmentAsset(jobId, `segment-voiceover-${segmentIndex}.wav`, payload.asset)
        : undefined;
    ...
    const speechBoundaryDuration = speechStartTime !== null && speechEndTime !== null && speechEndTime > speechStartTime
        ...
        : null;
    const speechDuration = speechBoundaryDuration ?? normalizeNumber(payload.speech_duration ?? payload.duration);
...
const normalizeStudioVoiceoverTextForMatch = (value) => normalizeGenerationText(value)
    .replace(/\s+/g, " ")
    .trim()
    .toLowerCase();
const isStudioVoiceoverSegmentTextCompatible = (statusText, requestedText) => {
...
export async function createStudioBatchVoiceoverJob(user, options) {
    assertAdsflowConfigured();
    cleanupStudioBatchVoiceoverFallbackJobs();
    const normalizedGroups = normalizeStudioBatchVoiceoverGroups(options?.groups);
    if (normalizedGroups.length === 0) {
        throw new Error("At least one voiceover group is required.");
    }
    const creditCost = getStudioBatchVoiceoverCreditCost(normalizedGroups);
    if (creditCost <= 0) {
...
export async function getStudioProjectVoiceoverJobStatus(jobId, user) {
    const payload = await fetchAdsflowProjectVoiceoverJobStatus(jobId, user);
    const status = String(payload.status ?? "queued").trim() || "queued";
    const resolvedJobId = String(payload.job_id ?? jobId).trim() || String(jobId ?? "").trim();
    const asset = payload.asset ? normalizeAdsflowProjectVoiceoverAsset(resolvedJobId, payload.asset) : undefined;
    const speechStartTime = normalizeNumber(payload.speech_start_time);
    const speechEndTime = normalizeNumber(payload.speech_end_time);
    const speechDuration = normalizeNumber(payload.speech_duration) ??
        (speechStartTime !== null && speechEndTime !== null ? Math.max(0, speechEndTime - speechStartTime) : null);
    const segments = (Array.isArray(payload.segments) ? payload.segments : [])
        ...
        const segmentSpeechBoundaryDuration = segmentSpeechStartTime !== null && segmentSpeechEndTime !== null && segmentSpeechEndTime > segmentSpeechStartTime
...
export async function getStudioBatchVoiceoverJobStatus(jobId, user) {
    cleanupStudioBatchVoiceoverFallbackJobs();
    const safeJobId = String(jobId ?? "").trim();
    if (!safeJobId) {
        throw new Error("Job id is required.");
    ...
    if (fallbackJob) {
        ...
        return {
            ...
            error: childStatuses.find((segment) => segment.error)?.error,
            jobId: safeJobId,
            profile,
            segments: childStatuses,
            status,
        };
    }
    const payload = await fetchAdsflowBatchVoiceoverJobStatus(safeJobId, user);
    const status = String(payload.status ?? "queued").trim() || "queued";
    const resolvedJobId = String(payload.job_id ?? safeJobId).trim() || safeJobId;
    const metadata = studioBatchVoiceoverJobMetadata.get(safeJobId) ?? studioBatchVoiceoverJobMetadata.get(resolvedJobId);
    ...
    const segments = rawSegments.map((segment, index) => {
        const segmentIndex = normalizeNonNegativeInteger(segment.segmentIndex ?? segment.segment_index) ?? index;
        const metadataGroup = metadata?.groups.find((group) => group.segments.some((metadataSegment) => metadataSegment.segmentIndex === segmentIndex));
        const metadataSegment = metadataGroup?.segments.find((item) => item.segmentIndex === segmentIndex);
        return normalizeAdsflowBatchVoiceoverStatusSegment(segment, {
            index,
            language: metadataGroup?.language,
            segmentIndex,
            text: metadataSegment?.text,
    ...
    return {
        ...
        error: normalizeGenerationText(payload.error) || segments.find((segment) => segment.error)?.error,
        jobId: resolvedJobId,
        profile: await enrichWorkspaceProfile(payload.user ?? undefined, {
            rawUserId: payload.user?.user_id ? String(payload.user.user_id) : undefined,
        }),
...
const getDirectSegmentAiPhotoJobStatus = async (jobId, user) => {
...
const isStudioGeneratedVideoReadyStatus = (status) => {
...
export async function getStudioProjectVoiceoverJobFileProxyTarget(jobId, user) {
    const safeJobId = String(jobId ?? "").trim();
    if (!safeJobId) {
        throw new Error("Job id is required.");
    }
    assertAdsflowConfigured();
    const externalUserId = await resolveStudioExternalUserId(user);
    return buildAdsflowUrl(`/api/web/project-voiceover/jobs/${encodeURIComponent(safeJobId)}/file`, {
        admin_token: env.adsflowAdminToken ?? "",
        external_user_id: externalUserId,
    });
}
