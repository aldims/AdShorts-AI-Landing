import { areWorkspaceMediaLibraryUrlsEqual, createWorkspaceMediaLibraryItem, getWorkspaceImageDownloadName, getWorkspaceProjectDisplayTitle, getWorkspaceVideoDownloadName, } from "../src/lib/workspaceMediaLibrary.js";
import { getWorkspaceProjects } from "./projects.js";
import { getWorkspaceSegmentEditorSessionForAccessibleProject, } from "./segment-editor.js";
...
const buildWorkspacePersistedMediaLibraryItems = (project, session) => {
    ...
    return session.segments.flatMap((segment, segmentListIndex) => {
        ...
        const hasAnimatedVariant = Boolean(currentPreviewUrl || currentPlaybackUrl) &&
            (!areWorkspaceMediaLibraryUrlsEqual(currentPreviewUrl, originalPreviewUrl) ||
                !areWorkspaceMediaLibraryUrlsEqual(currentPlaybackUrl, originalPlaybackUrl));
        if (hasAnimatedVariant) {
            ...
            if (animatedPreviewUrl) {
                items.push(createWorkspaceMediaLibraryItem({
                    ...
                    downloadUrl: appendUrlToken(currentPlaybackUrl ?? animatedPreviewUrl, "download", `${downloadToken}:${segment.index}:animation`),
...
const loadWorkspaceMediaLibraryItems = async (user, options) => {
    ...
    const results = await mapWithConcurrencyLimit(readyProjects, WORKSPACE_MEDIA_LIBRARY_SEGMENT_CONCURRENCY, async (project) => {
        try {
