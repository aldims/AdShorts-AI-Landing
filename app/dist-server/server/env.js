...
export const env = {
    ...
    openaiCharacterReferenceModel: trim(process.env.OPENAI_CHARACTER_REFERENCE_MODEL) ?? trim(process.env.OPENAI_IMAGE_MODEL) ?? "gpt-image-2",
    openaiCharacterReferenceQuality: trim(process.env.OPENAI_CHARACTER_REFERENCE_QUALITY) ?? "medium",
    openaiCharacterReferenceSize: trim(process.env.OPENAI_CHARACTER_REFERENCE_SIZE) ?? "2048x2048",
    openaiCharacterReferenceOutputFormat: trim(process.env.OPENAI_CHARACTER_REFERENCE_OUTPUT_FORMAT) ?? "png",
    openaiImageTimeoutMs: toNumber(process.env.OPENAI_IMAGE_TIMEOUT_MS, toNumber(process.env.OPENAI_IMAGE_TIMEOUT_SEC, 120) * 1000),
    ...
    studioSegmentInfographicEnabled: toBoolean(process.env.STUDIO_SEGMENT_INFOGRAPHIC_ENABLED, true),
