...
const sanitizeContentPlanIdeaPrompt = (value) => {
...
const toDatabaseBoolean = (value) => (isPgPool(database) ? value : value ? 1 : 0);
...
const normalizeContentPlanLanguage = (value) => normalizeText(value).toLowerCase() === "en" ? "en" : "ru";
const normalizeWorkspaceContentPlanIdeaSeeds = (ideas, positionOffset = 0) => ideas.map((idea, index) => ({
    id: randomUUID(),
...
const getWorkspaceContentPlanOwnerKey = (user) => {
...
let workspaceContentPlansTableReadyPromise = null;
...
const runInTransaction = async (callback) => {
    ...
    try {
        const result = await callback(async (sql, params = []) => sqliteDatabase.prepare(sql).all(...normalizeSqliteParams(params)), async (sql, params = []) => {
            sqliteDatabase.prepare(sql).run(...normalizeSqliteParams(params));
        });
...
const groupWorkspaceContentPlans = (rows) => {
    const plans = new Map();
    rows.forEach((row) => {
...
const getWorkspaceContentPlanById = async (ownerKey, planId) => {
    if (!ownerKey || !planId) {
        return null;
    }
    ...
    const sql = isPgPool(database)
      ...
      `
        ...
        ORDER BY i.position ASC
      `;
    const rows = await queryRows(sql, [ownerKey, planId]);
    return groupWorkspaceContentPlans(rows)[0] ?? null;
};
export async function getWorkspaceContentPlan(user, planId) {
...
export async function listWorkspaceContentPlans(user, limit = 24) {
    const ownerKey = getWorkspaceContentPlanOwnerKey(user);
    if (!ownerKey) {
        return [];
    }
    ...
    const sql = isPgPool(database)
      ...
      `
        ...
        ORDER BY p.updated_at DESC, p.created_at DESC, i.position ASC
      `;
    const rows = await queryRows(sql, [ownerKey, safeLimit]);
    return groupWorkspaceContentPlans(rows);
}
export async function createWorkspaceContentPlan(user, options) {
    const ownerKey = getWorkspaceContentPlanOwnerKey(user);
...
export async function appendWorkspaceContentPlanIdeas(user, options) {
    const ownerKey = getWorkspaceContentPlanOwnerKey(user);
    ...
    await runInTransaction(async (_query, run) => {
        ...
        await run(updatePlanSql, isPgPool(database) ? [ownerKey, normalizedPlanId, createdAt] : [createdAt, ownerKey, normalizedPlanId]);
    });
    return getWorkspaceContentPlanById(ownerKey, normalizedPlanId);
}
export async function updateWorkspaceContentPlanIdeaUsedState(user, options) {
    const ownerKey = getWorkspaceContentPlanOwnerKey(user);
    ...
    const selectSql = isPgPool(database)
      ...
      `
        ...
        LIMIT 1
      `;
    const matchingRows = await queryRows(selectSql, [ownerKey, planId, ideaId]);
...
export async function deleteWorkspaceContentPlanIdea(user, options) {
    const ownerKey = getWorkspaceContentPlanOwnerKey(user);
    const planId = normalizeText(options.planId);
    const ideaId = normalizeText(options.ideaId);
    if (!ownerKey || !planId || !ideaId) {
...
export async function deleteWorkspaceContentPlan(user, planId) {
    const ownerKey = getWorkspaceContentPlanOwnerKey(user);
    const normalizedPlanId = normalizeText(planId);
    if (!ownerKey || !normalizedPlanId) {
        return false;
    ...
    const selectSql = isPgPool(database)
      ...
      `
        ...
        LIMIT 1
      `;
    const existingRows = await queryRows(selectSql, [ownerKey, normalizedPlanId]);
