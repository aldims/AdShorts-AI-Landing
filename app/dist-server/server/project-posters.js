...
const projectPosterGenerationQueue = [];
const projectPosterGenerationRequests = new Map();
let activeProjectPosterGenerationCount = 0;
const FFMPEG_BINARY = process.env.FFMPEG_PATH?.trim() || "ffmpeg";
const normalizeText = (value) => String(value ?? "").replace(/\s+/g, " ").trim();
const getProjectPosterFileHash = (cacheKey) => createHash("sha256").update(cacheKey).digest("hex");
const getProjectPosterFilePath = (cacheKey) => join(PROJECT_POSTERS_ROOT_DIR, `${getProjectPosterFileHash(cacheKey)}.jpg`);
const projectPosterExists = async (cacheKey) => {
    try {
...
const flushProjectPosterGenerationQueue = () => {
    while (activeProjectPosterGenerationCount < PROJECT_POSTER_CAPTURE_CONCURRENCY &&
...
const enqueueProjectPosterGeneration = (task) => new Promise((resolve, reject) => {
    const runTask = () => {
        void task()
            .then(resolve)
            .catch(reject)
...
const runPosterCaptureCommand = async (upstreamUrl, frameTimeSeconds) => {
    const { stdout } = await execFileAsync(FFMPEG_BINARY, [
        "-hide_banner",
        "-loglevel",
        "error",
        "-y",
...
const capturePosterBuffer = async (upstreamUrl) => {
    let lastError = null;
...
const generateProjectPosterFile = async (source) => {
