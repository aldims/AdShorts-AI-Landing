...
const segmentVideoQueue = createAssetPreparationQueue({
...
const inferSegmentVideoContentType = (value, upstreamUrl) => {
...
const inferSegmentVideoExtension = (contentType, upstreamUrl) => {
...
const downloadWorkspaceSegmentVideo = async (source) => {
    ...
    const tempPath = join(SEGMENT_VIDEO_CACHE_ROOT_DIR, `${process.pid}-${Date.now()}-${randomUUID()}${extension}`);
    try {
...
const prepareWorkspaceSegmentVideoCacheAsset = async (source) => {
    ...
    const finalPath = await segmentVideoStore.commitFile(source.cacheKey, downloaded.tempPath, {
...
export async function peekWorkspaceSegmentVideoCacheAsset(cacheKey) {
    segmentVideoStore.scheduleCleanup();
    const cachedAsset = await segmentVideoStore.read(cacheKey);
    if (!cachedAsset) {
        return null;
    }
    return {
...
export async function ensureWorkspaceSegmentVideoCache(source) {
    segmentVideoStore.scheduleCleanup();
    const cachedAsset = await peekWorkspaceSegmentVideoCacheAsset(source.cacheKey);
    if (cachedAsset) {
        return cachedAsset;
