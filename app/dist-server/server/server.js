...
import { disconnectWorkspaceInstagramChannel, disconnectWorkspaceYoutubeChannel, getWorkspaceInstagramConnectUrl, getWorkspacePublishBootstrap, getWorkspacePublishJobStatus, getWorkspaceYoutubeConnectUrl, isWorkspacePublishSuccessStatus, normalizeWorkspacePublishPlatform, startWorkspaceInstagramPublish, startWorkspaceYoutubePublish, } from "./publish.js";
...
const studioSegmentInfographicCohort = new Set(String(env.studioSegmentInfographicCohort ?? "")
    ...
    .filter(Boolean));
const isStudioSegmentInfographicEnabledForUser = (user) => {
...
const normalizeWebDeviceId = (value) => {
...
const normalizeRequestPositiveInteger = (value) => {
...
const normalizeRequestPositiveIntegerList = (value) => {
...
const normalizeRequestNonNegativeInteger = (value) => {
    const numeric = Number(value);
    if (!Number.isFinite(numeric)) {
        return undefined;
    }
...
const normalizeRequestTalkingCharacterTarget = (value) => {
    ...
    const segmentEditor = segmentEditorRecord && rawSegments.length > 0
        ? {
            ...
            segments: await Promise.all(rawSegments.map(async (segment) => {
                ...
                const remoteUrl = typeof segmentRecord.customVideoRemoteUrl === "string"
                    ? segmentRecord.customVideoRemoteUrl.trim()
                    : "";
                const fallbackMimeType = typeof segmentRecord.customVideoFileMimeType === "string"
                ...
                return {
                    ...
                    sceneSoundAssetId: normalizeRequestPositiveInteger(segmentRecord.sceneSoundAssetId ?? segmentRecord.scene_sound_asset_id),
                    sceneSoundRemoved: normalizeRequestOptionalBoolean(segmentRecord.sceneSoundRemoved ?? segmentRecord.scene_sound_removed),
...
app.post("/api/auth/email-code/request", emailCodeRequestRateLimit, express.json({ limit: "32kb" }), async (req, res) => {
    ...
    try {
      ...
      `,
            ...
            text: `Ваш код входа в AdShorts AI: ${loginCode.code}. ` +
...
const readTelegramLoginField = (value) => {
...
const parseTelegramLoginData = (source) => {
    ...
    if (lastName)
        loginData.last_name = lastName;
    if (username)
        loginData.username = username;
    if (photoUrl)
        loginData.photo_url = photoUrl;
    return loginData;
};
...
const sendTelegramAuthPopupResult = (res, result) => {
...
app.get("/api/me", async (req, res) => {
    ...
    const telegramDisplay = await getTelegramAccountDisplay(session.user.id).catch((error) => {
        console.warn("[telegram] Failed to load Telegram account display", error);
        return null;
    });
    res.json(telegramDisplay
        ? {
            ...session,
            user: {
                ...session.user,
...
app.post("/api/workspace/content-plans/generate", express.json(), async (req, res) => {
    ...
    const requestedCount = typeof req.body?.count === "number"
        ? req.body.count
...
app.post("/api/referrals/click", async (req, res) => {
    ...
    if (body.human_interaction !== true) {
        res.json({ data: { code, ok: false, tracked: false, reason: "no_human_interaction" } });
        return;
    }
    try {
        const payload = await postAdsflowJson("/api/web/referral-click", {
            admin_token: env.adsflowAdminToken,
            code,
            human_interaction: true,
            interaction_type: typeof body.interaction_type === "string"
                ? body.interaction_type
                : undefined,
            web_device_id: normalizeWebDeviceId(body.web_device_id) || undefined,
            landing_url: typeof body.landing_url === "string" ? body.landing_url : undefined,
...
app.post("/api/studio/segment-talking-photo/jobs", async (req, res) => {
    ...
    const customVideoFileName = typeof req.body?.customVideoFileName === "string" ? req.body.customVideoFileName.trim() : "";
    const speakerTarget = normalizeRequestTalkingCharacterTarget(req.body?.speakerTarget ?? req.body?.speaker_target);
    const speakerConfirmationToken = typeof req.body?.speakerConfirmationToken === "string"
...
app.post("/api/studio/segment-scene-sound/jobs", async (req, res) => {
    ...
    const segmentIndex = normalizeRequiredJsonNonNegativeInteger(req.body?.segmentIndex ?? req.body?.segment_index);
    const durationSeconds = normalizeRequestDurationSeconds(req.body?.durationSeconds ?? req.body?.duration);
