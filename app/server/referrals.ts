...
type ReferralLinkRow = {
  assignedUserEmail: unknown;
  assignedUserId: unknown;
  code: unknown;
  createdAt: unknown;
  createdByUserEmail: unknown;
...
export type ReferralProductStats = {
  count: number;
  label: string;
  productId: string;
};

export type ReferralPurchaseSummary = {
...
export type ReferralLinkSummary = {
  ...
  stats: {
    clicks: number;
    products: ReferralProductStats[];
    purchases: number;
    recentPurchases: ReferralPurchaseSummary[];
    uniqueBuyers: number;
...
export type ReferralPurchaseInput = {
  balance?: unknown;
  code?: unknown;
  paymentId?: unknown;
  plan?: unknown;
...
const normalizeNullableEmail = (value: unknown) => normalizeEmail(value) || null;
...
const normalizeSqliteParams = (params: readonly unknown[]) => params.map((param) => param ?? null);
...
let referralTablesReadyPromise: Promise<void> | null = null;
...
export const isReferralAdminUser = (user: ReferralUser | null | undefined) =>
...
const createGeneratedReferralCode = () =>
  Array.from({ length: 10 }, () => REFERRAL_CODE_CHARS[Math.floor(Math.random() * REFERRAL_CODE_CHARS.length)]).join("");
...
const getProductLabel = (productId: string) => productLabels[productId] ?? productId.toUpperCase();

const rowToLink = (row: ReferralLinkRow): Omit<ReferralLinkSummary, "stats"> => {
...
const rowToEvent = (row: ReferralEventRow) => ({
...
const getReferralLinkByCode = async (code: string) => {
  await ensureReferralTables();

  const rows = await queryRows<ReferralLinkRow>(
    isPgPool(database)
      ? `
          SELECT
            id,
...
const buildIdentityKey = (event: ReturnType<typeof rowToEvent>) =>
  event.authUserId || event.authUserEmail || event.webDeviceId || event.id;

const buildLinkStats = (linkId: string, events: ReturnType<typeof rowToEvent>[]): ReferralLinkSummary["stats"] => {
...
export async function listReferralLinks(): Promise<ReferralLinkSummary[]> {
  await ensureReferralTables();

  const linkRows = await queryRows<ReferralLinkRow>(`
    SELECT
      id,
      code,
      label,
  ...
  const eventRows = await queryRows<ReferralEventRow>(`
    SELECT
      id,
      link_id AS "linkId",
      code,
...
export async function createReferralLink(input: ReferralLinkInput, user: ReferralUser): Promise<ReferralLinkSummary> {
  await ensureReferralTables();
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    ...
    try {
      ...
      return (await listReferralLinks()).find((link) => link.code === code) as ReferralLinkSummary;
    } catch (error) {
...
export async function updateReferralLink(
  linkId: string,
  input: ReferralLinkUpdateInput,
): Promise<ReferralLinkSummary> {
...
export async function recordReferralClick(
  input: ReferralClickInput,
  user?: ReferralUser | null,
): Promise<ReferralLinkSummary | null> {
...
export async function recordReferralPurchase(
  ...
  if (paymentId) {
    const existingRows = await queryRows<QueryRow>(
      isPgPool(database)
        ? "SELECT id FROM referral_events WHERE link_id = $1 AND payment_id = $2 AND type = 'purchase' LIMIT 1"
        : "SELECT id FROM referral_events WHERE link_id = ? AND payment_id = ? AND type = 'purchase' LIMIT 1",
      [link.id, paymentId],
    );
...
export const readReferralCodeFromCookieHeader = (cookieHeader: unknown) => {
