...
const playbackQueue = createAssetPreparationQueue<WorkspaceProjectPlaybackAsset>({
...
const prepareWorkspaceProjectPlaybackAsset = async (
  ...
  const finalPath = await playbackStore.commitFile<WorkspaceProjectPlaybackMetadata>(
    source.cacheKey,
    tempOutputPath,
    {
...
const ensureWorkspaceProjectPlaybackQueued = (
  ...
  playbackQueue.schedule(source.cacheKey, priority, async () => {
...
export async function peekWorkspaceProjectPlaybackAsset(
  cacheKey: string,
): Promise<WorkspaceProjectPlaybackAsset | null> {
  playbackStore.scheduleCleanup();
  const cachedAsset = await playbackStore.read<WorkspaceProjectPlaybackMetadata>(cacheKey);
  if (!cachedAsset) {
    return null;
  }
