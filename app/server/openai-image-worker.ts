...
type OpenAIImageEditOptions = OpenAIImageGenerationOptions & {
  image: Buffer;
...
export class OpenAIImageProviderError extends Error {
  ...
  constructor(message: string, options?: { fallbackable?: boolean; policy?: boolean; status?: number }) {
    super(message);
...
export const isOpenAIImagePolicyError = (error: unknown) =>
  error instanceof OpenAIImageProviderError && error.policy;
...
const normalizeModel = (value: unknown) => normalizeText(value) || "gpt-image-2";
const normalizeQuality = (value: unknown, fallback: OpenAIImageQuality): OpenAIImageQuality => {
  const normalized = normalizeText(value).toLowerCase();
  return normalized === "low" || normalized === "medium" || normalized === "high" || normalized === "auto"
    ? normalized
    : fallback;
};
const normalizeOutputFormat = (value: unknown, fallback: OpenAIImageOutputFormat): OpenAIImageOutputFormat => {
  const normalized = normalizeText(value).toLowerCase();
  return normalized === "png" || normalized === "jpeg" || normalized === "webp" ? normalized : fallback;
};

const getOpenAIImageMimeType = (format: OpenAIImageOutputFormat) => {
  if (format === "jpeg") return "image/jpeg";
  if (format === "webp") return "image/webp";
  return "image/png";
};

const getOpenAIErrorMessage = (payload: OpenAIImagePayload | null, fallback: string) =>
  normalizeText(payload?.error?.message) || fallback;

const isOpenAIPolicyPayload = (status: number, payload: OpenAIImagePayload | null) => {
...
const parseOpenAIImagePayload = async (response: Response) => {
  try {
...
const buildOpenAIImageError = (response: Response, payload: OpenAIImagePayload | null, operation: string) => {
  ...
  return new OpenAIImageProviderError(
    getOpenAIErrorMessage(payload, `OpenAI ${operation} failed (${response.status}).`),
    {
      fallbackable,
      policy,
...
const decodeOpenAIImage = (
  payload: OpenAIImagePayload | null,
  options: {
    model: string;
    outputFormat: OpenAIImageOutputFormat;
    quality: OpenAIImageQuality;
...
const getCharacterReferenceOptions = (options: OpenAIImageGenerationOptions) => {
...
export const createOpenAIImageEdit = async (
  ...
  formData.append(
    "image[]",
    new Blob([new Uint8Array(options.image)], {
