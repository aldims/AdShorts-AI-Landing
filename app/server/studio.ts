...
type AdsflowProjectMediaResponse = {
  assets?: AdsflowMediaAssetPayload[] | null;
...
type AdsflowJobStatusResponse = {
  ...
  download_url?: string | null;
  error?: string | null;
  final_asset?: AdsflowMediaAssetPayload | null;
  final_video_asset?: AdsflowMediaAssetPayload | null;
  generated_at?: string | null;
  hashtags?: string | null;
  job_id?: string;
  media_asset?: AdsflowMediaAssetPayload | null;
  media_asset_id?: number | string | null;
  prompt?: string | null;
  ...
  title?: string | null;
  url?: string | null;
  video_asset?: AdsflowMediaAssetPayload | null;
...
type AdsflowBatchVoiceoverSegmentStatusPayload = AdsflowSegmentVoiceoverJobStatusResponse & {
  ...
  segmentIndex?: number | string | null;
  segment_index?: number | string | null;
  target_duration?: number | string | null;
  text?: string | null;
  voiceType?: string | null;
  voice_type?: string | null;
};

type AdsflowBatchVoiceoverJobStatusResponse = AdsflowSegmentAiVideoJobCreateResponse & {
  error?: string | null;
  results?: AdsflowBatchVoiceoverSegmentStatusPayload[] | null;
  segments?: AdsflowBatchVoiceoverSegmentStatusPayload[] | null;
};
...
export type StudioProjectVoiceoverSegment = {
  segmentIndex: number;
  targetDurationSeconds?: number | null;
  text: string;
};

export type StudioProjectVoiceoverJob = StudioSegmentVoiceoverJob;

export type StudioBatchVoiceoverSegment = StudioProjectVoiceoverSegment;

export type StudioBatchVoiceoverGroup = {
  language?: string;
  segments: StudioBatchVoiceoverSegment[];
  voiceType?: string | null;
};

export type StudioBatchVoiceoverJob = StudioSegmentVoiceoverJob & {
  creditCost: number;
};
...
export type StudioBatchVoiceoverJobStatus = {
  creditCost: number;
  error?: string;
  jobId: string;
  profile: WorkspaceProfile;
...
export const resolveStudioSegmentEditorGenerationMediaFlags = (options: {
  requestedSubtitleEnabled?: boolean;
  requestedVoiceEnabled: boolean;
  segmentEditor?: StudioSegmentEditorPayload | null;
}) => {
  const segmentEditorHasVoice =
    options.segmentEditor?.segments.some(
      (segment) =>
        Boolean(segment.voiceoverAssetId) ||
        Boolean(segment.voiceType && segment.voiceType !== "none"),
    ) ?? false;
  const segmentEditorHasSubtitles =
    options.segmentEditor?.segments.some(
      (segment) => Boolean(segment.subtitleType && segment.subtitleType !== "none"),
    ) ?? false;
  const voiceEnabled = options.requestedVoiceEnabled || segmentEditorHasVoice;
  const subtitleEnabled =
    voiceEnabled && (options.requestedSubtitleEnabled !== false || segmentEditorHasSubtitles);
...
const normalizeStudioSegmentVoiceoverText = (value: unknown) => {
...
const isStudioVoiceSupportedForLanguage = (voiceId: string | null | undefined, language: Locale) => {
  const canonicalVoiceId = getCanonicalStudioVoiceId(voiceId);
  return Boolean(
    canonicalVoiceId &&
...
const normalizeAdsflowFinalVideoCandidatePath = (value: unknown) => {
  const normalized = normalizeGenerationText(value);
...
const getAdsflowFinalVideoAssetPayloads = (payload: AdsflowFinalVideoPayload | null | undefined) =>
  ADSFLOW_FINAL_VIDEO_ASSET_FIELDS
    ...
    .filter((value): value is AdsflowMediaAssetPayload => Boolean(value && typeof value === "object"));

const resolveAdsflowFinalVideoMediaAssetId = (payload: AdsflowFinalVideoPayload | null | undefined) => {
...
export const resolveAdsflowFinalVideoDownloadPath = (
  payload: AdsflowFinalVideoPayload | null | undefined,
  fallbackDownloadPath?: string | null,
) => {
  const rootCandidate = [
...
const normalizeAdsflowMediaAssetClassifier = (value: unknown) =>
  normalizeGenerationText(value)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "");

const isAdsflowVideoMediaAsset = (asset: AdsflowMediaAssetPayload | null | undefined) => {
...
const isAdsflowReadyMediaAsset = (asset: AdsflowMediaAssetPayload | null | undefined) => {
...
const isAdsflowFinalVideoMediaAsset = (asset: AdsflowMediaAssetPayload | null | undefined) => {
  ...
  const hasFinalClassifier = classifiers.some((classifier) =>
    classifier === "final_video" ||
...
const compareAdsflowMediaAssetRecency = (left: AdsflowMediaAssetPayload, right: AdsflowMediaAssetPayload) => {
...
const pickAdsflowProjectFinalVideoAsset = (
  assets: AdsflowMediaAssetPayload[] | null | undefined,
  jobId?: string | null,
) => {
...
const buildStudioBatchVoiceoverSegmentAudioUrl = (
...
const buildStudioProjectVoiceoverJobAudioProxyUrl = (jobId: string | null | undefined) => {
  const normalizedJobId = normalizeGenerationText(jobId);
  if (!normalizedJobId) {
    return null;
  }
...
const normalizeAdsflowBatchVoiceoverSegmentAsset = (
  jobId: string | null | undefined,
...
const normalizeAdsflowProjectVoiceoverAsset = (
  jobId: string,
  payload?: AdsflowSegmentAiVideoAssetPayload | null,
): StudioGeneratedAudioAsset => {
...
const syncStudioGenerationStatusToHistory = async (
  user: StudioUser,
  status: StudioGenerationStatus | null | undefined,
  options?: {
    fallbackEntry?: WorkspaceGenerationHistoryEntry | null;
  },
) => {
...
const fetchAdsflowProjectMedia = async (projectId: number, user: StudioUser) => {
  ...
  return fetchAdsflowJson<AdsflowProjectMediaResponse>(
    buildAdsflowUrl(`/api/projects/${encodeURIComponent(String(safeProjectId))}/media`, {
      admin_token: env.adsflowAdminToken ?? "",
      external_user_id: externalUserId,
    }),
  );
...
const fetchAdsflowProjectFinalVideoAsset = async (
...
const fetchAdsflowProjectVoiceoverJobStatus = async (jobId: string, user: StudioUser) => {
  assertAdsflowConfigured();
  ...
  return fetchAdsflowJson<AdsflowSegmentVoiceoverJobStatusResponse>(
    buildAdsflowUrl(`/api/web/project-voiceover/jobs/${encodeURIComponent(safeJobId)}`, {
      admin_token: env.adsflowAdminToken ?? "",
      external_user_id: externalUserId,
    }),
  );
...
const fetchAdsflowBatchVoiceoverJobStatus = async (jobId: string, user: StudioUser) => {
  assertAdsflowConfigured();
  ...
  return fetchAdsflowJson<AdsflowBatchVoiceoverJobStatusResponse>(
    buildAdsflowUrl(`/api/web/voiceover/batch-jobs/${encodeURIComponent(safeJobId)}`, {
      admin_token: env.adsflowAdminToken ?? "",
      external_user_id: externalUserId,
    }),
  );
...
export async function createStudioGenerationJob(
  ...
  const normalizedSegmentEditor = normalizeStudioSegmentEditorPayload(
    ...
    { globalVoiceEnabled: requestedVoiceEnabled },
  );
...
export async function createStudioSegmentPhotoAnimationJob(
  ...
  const normalizedDurationExtensionSourceVideoFileDataUrl =
    String(options?.durationExtensionSourceVideoFileDataUrl ?? "").trim() || undefined;
  const normalizedDurationExtensionSourceVideoFileMimeType =
    String(options?.durationExtensionSourceVideoFileMimeType ?? "").trim() || undefined;
  const normalizedDurationExtensionSourceVideoFileName =
    String(options?.durationExtensionSourceVideoFileName ?? "").trim() || undefined;
  const externalUserId = await resolveStudioExternalUserId(user);
  const subscriptionDetails = await fetchAdsflowSubscriptionDetailsForWebMutation(externalUserId, user);
  const customVideoAssetId =
    normalizedCustomVideoAssetId
      ...
      : undefined;
  const durationExtensionSourceVideoAssetId =
...
export async function createStudioProjectVoiceoverJob(
  text: string,
  user: StudioUser,
  options?: {
    language?: string;
    projectId?: number;
    segments?: StudioProjectVoiceoverSegment[];
    voiceType?: string | null;
  },
): Promise<StudioProjectVoiceoverJob> {
  assertAdsflowConfigured();
  ...
  let payload: AdsflowSegmentAiVideoJobCreateResponse;
  try {
    payload = await postAdsflowJson<AdsflowSegmentAiVideoJobCreateResponse>("/api/web/project-voiceover/jobs", {
      admin_token: env.adsflowAdminToken,
      credit_cost: requiredCredits,
      external_user_id: externalUserId,
      language: normalizedLanguage,
...
type NormalizedStudioBatchVoiceoverSegment = {
  segmentIndex: number;
  targetDurationSeconds: number | null;
  text: string;
};

type NormalizedStudioBatchVoiceoverGroup = {
  creditCost: number;
  language: "en" | "ru";
  segments: NormalizedStudioBatchVoiceoverSegment[];
  voiceType: string;
};

type StudioBatchVoiceoverFallbackChildJob = NormalizedStudioBatchVoiceoverSegment & {
  jobId: string;
  language: "en" | "ru";
  voiceType: string;
};
...
type StudioBatchVoiceoverJobMetadata = {
  createdAt: number;
  creditCost: number;
  groups: NormalizedStudioBatchVoiceoverGroup[];
};

const studioBatchVoiceoverFallbackJobs = new Map<string, StudioBatchVoiceoverFallbackJob>();
const studioBatchVoiceoverJobMetadata = new Map<string, StudioBatchVoiceoverJobMetadata>();
...
const isStudioVoiceoverReadyStatus = (value: unknown) => {
...
const normalizeStudioBatchVoiceoverGroups = (
  groups: StudioBatchVoiceoverGroup[] | null | undefined,
): NormalizedStudioBatchVoiceoverGroup[] => {
  const normalizedGroups = (groups ?? [])
    ...
    .filter((group): group is NormalizedStudioBatchVoiceoverGroup => Boolean(group));

  const mergedGroups = new Map<string, NormalizedStudioBatchVoiceoverGroup>();
...
const normalizeAdsflowBatchVoiceoverStatusSegment = (
...
): StudioBatchVoiceoverJobSegmentStatus => {
  const segmentIndex =
    normalizeNonNegativeInteger(payload.segmentIndex ?? payload.segment_index) ??
  ...
  const asset = payload.asset
    ? normalizeAdsflowBatchVoiceoverSegmentAsset(
        jobId,
        `segment-voiceover-${segmentIndex}.wav`,
        payload.asset,
      )
    : undefined;
...
export async function createStudioBatchVoiceoverJob(
  user: StudioUser,
  options?: {
    groups?: StudioBatchVoiceoverGroup[];
    projectId?: number | null;
  },
): Promise<StudioBatchVoiceoverJob> {
  assertAdsflowConfigured();
...
export async function getStudioProjectVoiceoverJobStatus(
  jobId: string,
  user: StudioUser,
): Promise<StudioProjectVoiceoverJobStatus> {
...
export async function getStudioBatchVoiceoverJobStatus(
  jobId: string,
  user: StudioUser,
): Promise<StudioBatchVoiceoverJobStatus> {
  cleanupStudioBatchVoiceoverFallbackJobs();
  ...
  const segments = rawSegments.map((segment, index) => {
    const segmentIndex = normalizeNonNegativeInteger(segment.segmentIndex ?? segment.segment_index) ?? index;
    const metadataGroup = metadata?.groups.find((group) =>
      group.segments.some((metadataSegment) => metadataSegment.segmentIndex === segmentIndex),
    );
...
export async function getStudioGenerationStatus(jobId: string, user: StudioUser): Promise<StudioGenerationStatus> {
  ...
  let projectFinalVideoAsset: AdsflowMediaAssetPayload | null = null;
...
export async function getStudioProjectVoiceoverJobFileProxyTarget(jobId: string, user: StudioUser): Promise<URL> {
  const safeJobId = String(jobId ?? "").trim();
  if (!safeJobId) {
    throw new Error("Job id is required.");
  }
