...
type AuthTable = keyof typeof AUTH_TABLES;
type AuthRow = Record<string, unknown>;

const AUTH_TABLE_ORDER: AuthTable[] = ["user", "account", "session", "verification"];
...
const loadSourceRows = (sourceDb: DatabaseSync, table: AuthTable): AuthRow[] => {
  ...
  return statement.all() as AuthRow[];
};
...
const copyToSqlite = (targetDb: DatabaseSync, table: AuthTable, rows: AuthRow[]) => {
  if (!rows.length) return;

  const columns = AUTH_TABLES[table];
  const placeholders = columns.map(() => "?").join(", ");
  const statement = targetDb.prepare(
    `INSERT OR IGNORE INTO ${quoteIdentifier(table)} (${columns.map((column) => quoteIdentifier(column)).join(", ")}) VALUES (${placeholders})`,
  );
...
const copyToPostgres = async (targetDb: Pool, table: AuthTable, rows: AuthRow[]) => {
  if (!rows.length) return;
...
const countSourceRows = (sourceDb: DatabaseSync, table: AuthTable) => {
  const result = sourceDb.prepare(`SELECT COUNT(*) AS total FROM ${quoteIdentifier(table)}`).get() as {
    total?: number;
  };
