...
export type WorkspaceProjectSegmentPlaybackClipSource = {
  cacheKey: string;
  duration: number;
  inputPath: string;
  projectId: number;
...
export type WorkspaceProjectSegmentPlaybackAsset = {
  absolutePath: string;
  contentType: string;
};
...
type WorkspaceProjectSegmentPlaybackMetadata = PreparedAssetMetadata;
...
const segmentPlaybackQueue = createAssetPreparationQueue<WorkspaceProjectSegmentPlaybackAsset>({
...
const recentSegmentPlaybackFailures = new Map<string, number>();

const normalizeText = (value: unknown) => String(value ?? "").replace(/\s+/g, " ").trim();

const normalizeSegmentBoundary = (value: number) => {
  const numeric = Number.isFinite(value) ? Math.max(0, value) : 0;
  return Number(numeric.toFixed(3));
};

const formatFfmpegTime = (value: number) => normalizeSegmentBoundary(value).toFixed(3);
...
const hasRecentWorkspaceProjectSegmentPlaybackFailure = (cacheKey: string) => {
...
const markWorkspaceProjectSegmentPlaybackFailure = (cacheKey: string) => {
  recentSegmentPlaybackFailures.set(cacheKey, Date.now() + PROJECT_SEGMENT_PLAYBACK_FAILURE_TTL_MS);
};

const clearWorkspaceProjectSegmentPlaybackFailure = (cacheKey: string) => {
  recentSegmentPlaybackFailures.delete(cacheKey);
};

const clipWorkspaceProjectSegmentPlaybackFile = async (
  source: WorkspaceProjectSegmentPlaybackClipSource,
  outputPath: string,
) => {
  await execFileAsync(
    FFMPEG_BINARY,
...
const prepareWorkspaceProjectSegmentPlaybackAsset = async (
  source: WorkspaceProjectSegmentPlaybackClipSource,
): Promise<WorkspaceProjectSegmentPlaybackAsset> => {
  ...
  const tempOutputPath = join(
    PROJECT_SEGMENT_PLAYBACK_ROOT_DIR,
    `${process.pid}-${Date.now()}-${randomUUID()}-segment.mp4`,
  );

  try {
    await clipWorkspaceProjectSegmentPlaybackFile(source, tempOutputPath);
    const finalPath = await segmentPlaybackStore.commitFile<WorkspaceProjectSegmentPlaybackMetadata>(
      source.cacheKey,
      tempOutputPath,
      {
...
const ensureWorkspaceProjectSegmentPlaybackQueued = (
  source: WorkspaceProjectSegmentPlaybackClipSource,
  priority: AssetPreparationPriority,
) =>
  segmentPlaybackQueue.schedule(source.cacheKey, priority, async () => {
...
export async function peekWorkspaceProjectSegmentPlaybackAsset(
  cacheKey: string,
): Promise<WorkspaceProjectSegmentPlaybackAsset | null> {
  segmentPlaybackStore.scheduleCleanup();
  const cachedAsset = await segmentPlaybackStore.read<WorkspaceProjectSegmentPlaybackMetadata>(cacheKey);
  if (!cachedAsset) {
    return null;
  }
...
export async function ensureWorkspaceProjectSegmentPlayback(
  source: WorkspaceProjectSegmentPlaybackClipSource,
): Promise<WorkspaceProjectSegmentPlaybackAsset> {
  segmentPlaybackStore.scheduleCleanup();
