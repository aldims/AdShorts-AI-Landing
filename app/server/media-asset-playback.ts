...
export type WorkspaceMediaAssetPlaybackSource = {
  assetId: number;
  cacheKey: string;
  upstreamUrl: URL;
};

export type WorkspaceMediaAssetPlaybackAsset = {
  absolutePath: string;
  contentType: string;
};
...
type WorkspaceMediaAssetPlaybackMetadata = PreparedAssetMetadata;
...
const mediaAssetPlaybackQueue = createAssetPreparationQueue<WorkspaceMediaAssetPlaybackAsset>({
...
const recentMediaAssetPlaybackFailures = new Map<string, number>();

const normalizeText = (value: unknown) => String(value ?? "").replace(/\s+/g, " ").trim();

const hasRecentWorkspaceMediaAssetPlaybackFailure = (cacheKey: string) => {
...
const isWorkspaceMediaAssetPlaybackRetryableError = (error: unknown) => {
...
const markWorkspaceMediaAssetPlaybackFailure = (cacheKey: string, error?: unknown) => {
...
const clearWorkspaceMediaAssetPlaybackFailure = (cacheKey: string) => {
  recentMediaAssetPlaybackFailures.delete(cacheKey);
};

export const inferWorkspaceMediaAssetPlaybackContentType = (value: string | null | undefined, upstreamUrl: URL) => {
...
export const inferWorkspaceMediaAssetPlaybackExtension = (contentType: string, upstreamUrl: URL) => {
...
const isWorkspaceMediaAssetPlaybackVideoContentType = (contentType: string) =>
  normalizeText(contentType).toLowerCase().startsWith("video/");

const downloadWorkspaceMediaAssetPlaybackSource = async (source: WorkspaceMediaAssetPlaybackSource) => {
  ...
  const contentType = inferWorkspaceMediaAssetPlaybackContentType(
...
const remuxWorkspaceMediaAssetPlaybackFile = async (inputPath: string, outputPath: string) => {
  await execFileAsync(
    FFMPEG_BINARY,
    [
      "-hide_banner",
...
const prepareWorkspaceMediaAssetPlaybackAsset = async (
  source: WorkspaceMediaAssetPlaybackSource,
): Promise<WorkspaceMediaAssetPlaybackAsset> => {
  ...
  const finalPath = await mediaAssetPlaybackStore.commitFile<WorkspaceMediaAssetPlaybackMetadata>(
    source.cacheKey,
    tempOutputPath,
    {
...
const ensureWorkspaceMediaAssetPlaybackQueued = (
  source: WorkspaceMediaAssetPlaybackSource,
  priority: AssetPreparationPriority,
) =>
  mediaAssetPlaybackQueue.schedule(source.cacheKey, priority, async () => {
...
export async function peekWorkspaceMediaAssetPlaybackAsset(
  cacheKey: string,
): Promise<WorkspaceMediaAssetPlaybackAsset | null> {
  mediaAssetPlaybackStore.scheduleCleanup();
  const cachedAsset = await mediaAssetPlaybackStore.read<WorkspaceMediaAssetPlaybackMetadata>(cacheKey);
  if (!cachedAsset) {
    return null;
  }
...
export async function ensureWorkspaceMediaAssetPlayback(
  source: WorkspaceMediaAssetPlaybackSource,
): Promise<WorkspaceMediaAssetPlaybackAsset> {
  mediaAssetPlaybackStore.scheduleCleanup();
...
export async function warmWorkspaceMediaAssetPlayback(source: WorkspaceMediaAssetPlaybackSource): Promise<void> {
