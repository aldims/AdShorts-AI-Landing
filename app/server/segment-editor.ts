...
import {
  ...
  type WorkspaceSegmentSpeechTimingCoordinateSpace,
  type WorkspaceSegmentVoiceSourceCoordinateSpace,
} from "../src/lib/workspaceSegmentEditorTimeline.js";
...
type AdsflowSegmentEditorSegmentPayload = {
  ...
  _voice_source_coordinate_space?: string | null;
  ...
  voiceSourceCoordinateSpace?: string | null;
  voice_source_coordinate_space?: string | null;
  ...
  providerTimingCoordinateSpace?: string | null;
  provider_timing_coordinate_space?: string | null;
  wordTimingCoordinateSpace?: string | null;
  word_timing_coordinate_space?: string | null;
  ...
  speechTimingCoordinateSpace?: string | null;
  speech_timing_coordinate_space?: string | null;
  start_time?: number | string | null;
...
type AdsflowSegmentRenderContractSegmentPayload = {
...
type AdsflowProjectGenerationSettingsPayload = {
  ...
  final_asset_id?: number | string | null;
  final_video_invalidated_at?: string | null;
  final_video_stale?: boolean | string | number | null;
  ...
  segment_render_contract?: {
    segments?: AdsflowSegmentRenderContractSegmentPayload[] | null;
  } | null;
...
const roundSegmentEditorTimelineSeconds = (value: number) => Math.round(value * 1000) / 1000;
...
const isProjectVoiceoverMediaEntry = (entry?: AdsflowProjectMediaEntryPayload | null) => {
  const roleText = getProjectMediaEntryRoleText(entry).replace(/-/g, "_");
  const hasVoiceoverRole = (
    roleText.includes("voiceover") ||
    roleText.includes("segment_voice") ||
    roleText.includes("segment_tts") ||
    roleText.includes("tts")
  );
  return hasVoiceoverRole && (
    isProjectMediaEntryAudio(entry) ||
    normalizePositiveProjectId(getProjectMediaEntryAssetId(entry)) !== null
  );
};
...
const buildProjectMediaEntriesBySegmentIndex = (
  entries: AdsflowProjectMediaEntryPayload[],
  expectedSegmentIndexes: number[],
  sourceName: string,
) => {
  const entriesBySegmentIndex = new Map<number, AdsflowProjectMediaEntryPayload>();
  const unindexedEntries: AdsflowProjectMediaEntryPayload[] = [];
  ...
  for (const entry of segmentEntries) {
    ...
    if (entriesBySegmentIndex.has(segmentIndex)) {
      throw new WorkspaceSegmentEditorError(
        `Источник ${sourceName} содержит несколько медиа для сегмента ${segmentIndex + 1}.`,
        409,
      );
    }
  ...
  if (entriesBySegmentIndex.size > 0) {
    if (unindexedEntries.length > 0) {
      throw new WorkspaceSegmentEditorError(
        `Источник ${sourceName} смешивает медиа с segment_index и без него.`,
        409,
      );
    }
  ...
  if (
    segmentEntries.length !== expectedSegmentIndexes.length ||
...
const findProjectMediaEntryBySegmentIndex = (
  entries: AdsflowProjectMediaEntryPayload[],
  segmentIndex: number,
) => entries.find((entry) => normalizeInteger(entry.segment_index) === segmentIndex) ?? null;

const assertUniqueSegmentEditorIndexes = (indexes: number[], sourceName: string) => {
  const seenIndexes = new Set<number>();
  for (const index of indexes) {
    if (seenIndexes.has(index)) {
      throw new WorkspaceSegmentEditorError(
        `Источник ${sourceName} содержит повторяющийся индекс сегмента ${index}.`,
        409,
      );
    }
...
const getProjectSegmentRenderContractText = (
  segment: AdsflowSegmentEditorSegmentPayload | null | undefined,
) => String(segment?.text ?? segment?.speech_text ?? "").trim();

const recoverProjectVoiceoverEntriesFromRenderContract = (
  settings: AdsflowProjectGenerationSettingsPayload | null | undefined,
): AdsflowProjectMediaEntryPayload[] => {
  if (!settings) {
    return [];
  }

  const declaredEntries = normalizeProjectMediaEntries(settings.segment_voiceover_assets);
  const hasDeclaredVoiceoverAsset = declaredEntries.some((entry) => (
    normalizePositiveProjectId(getProjectMediaEntryAssetId(entry)) !== null ||
    Boolean(normalizeText(entry.download_url) || normalizeText(entry.url))
  ));
  if (hasDeclaredVoiceoverAsset) {
    return declaredEntries;
  }

  const contractSegments = settings.segment_render_contract?.segments;
  ...
  const matchingSegments = candidateSegmentLists.find((candidate) => {
    ...
    return contractSegments.every((contractSegment, index) => {
      if (!contractSegment || typeof contractSegment !== "object") {
        return false;
      }

      const expectedHash = normalizeText(contractSegment.text_sha256).toLowerCase();
      if (!/^[0-9a-f]{64}$/.test(expectedHash)) {
        return false;
      }

      const currentText = getProjectSegmentRenderContractText(candidate[index]);
      const currentHash = createHash("sha256").update(currentText, "utf8").digest("hex");
  ...
  return contractSegments.flatMap((contractSegment, index) => {
    const voiceoverAssetId = normalizePositiveProjectId(contractSegment.voice?.voiceover_asset_id);
    if (voiceoverAssetId === null) {
      return [];
    }

    const segmentIndex = normalizeInteger(contractSegment.segment_index) ?? index;
    return [{
      library_kind: "segment_voiceover",
      media_asset_id: voiceoverAssetId,
      media_type: "audio",
      mime_type: "audio/wav",
      role: "segment_voiceover",
      segment_index: segmentIndex,
      source: "segment_voiceover",
      source_kind: "ai_voiceover",
    } satisfies AdsflowProjectMediaEntryPayload];
  });
};
...
const mergeProjectDetailTimelineIntoSegmentEditorPayload = (
  ...
  const authoritativeRecord = authoritativeSegment as Record<string, unknown>;
  const segmentRecord = segment as Record<string, unknown>;
  const hasOwnTimingField = (record: Record<string, unknown>, fields: readonly string[]) =>
  ...
  const speechWords = Array.isArray(speechTimingRecord.speech_words)
    ? speechTimingRecord.speech_words as AdsflowSegmentEditorSpeechWordPayload[]
    : null;
...
const getExplicitSegmentEditorSegmentIndex = (
  segment: AdsflowSegmentEditorSegmentPayload | null | undefined,
) => {
  const record = (segment ?? {}) as Record<string, unknown>;
...
const inferSegmentEditorPayloadProjectVoiceType = (
  payload: AdsflowSegmentEditorResponse,
  ...
  const candidateCounts = new Map<string, { count: number; value: string }>();
...
const pickSegmentEditorSpeechTimingCoordinateSpace = (
...
const pickSegmentEditorVoiceSourceCoordinateSpace = (
  record: Record<string, unknown> | null | undefined,
) => {
  for (const value of [
...
const hydrateSegmentEditorPayloadWithInheritedMusic = (
  payload: AdsflowSegmentEditorResponse,
  projectDetailsPayload: AdsflowProjectDetailsResponse | null | undefined,
  sourcePayload: AdsflowSegmentEditorResponse | null | undefined,
  ...
  const effectiveVoiceType = inferSegmentEditorPayloadProjectVoiceType(
    payload,
    normalizeText(payload.voice_type),
  );
  const effectiveVoiceKey = normalizeSegmentEditorComparableText(effectiveVoiceType);
  ...
  return {
    ...
    segments: (payload.segments ?? []).map((segment) => ({
      ...segment,
      voice_type:
        effectiveVoiceKey && normalizeSegmentEditorComparableText(segment.voice_type) === effectiveVoiceKey
          ? null
          : segment.voice_type,
    })),
...
const buildSegmentEditorPayloadFromProjectDetails = (
  ...
  const originalEntriesBySegmentIndex = buildProjectMediaEntriesBySegmentIndex(
    originalEntries,
    segmentIndexes,
    "исходных медиа проекта",
  );
  const currentEntriesBySegmentIndex = buildProjectMediaEntriesBySegmentIndex(
    currentEntries,
    segmentIndexes,
    "текущих медиа проекта",
  );
  const projectId =
...
const tokenizeSegmentEditorSpeechBoundaryText = (value: unknown) =>
  normalizeText(value)
    .toLowerCase()
    .split(/[^\p{L}\p{N}]+/gu)
    .filter(Boolean);

const getSegmentEditorSpeechWordTokenItems = (speechWords: WorkspaceSegmentEditorSpeechWord[]) =>
  speechWords.flatMap((word, wordIndex) =>
    tokenizeSegmentEditorSpeechBoundaryText(word.text).map((token) => ({
      token,
      wordIndex,
    })),
  );

const doSegmentEditorTokensEqual = (left: string[], right: string[]) =>
  left.length === right.length && left.every((token, index) => token === right[index]);

const isSegmentEditorTokenPrefix = (prefix: string[], tokens: string[]) =>
...
const cloneWorkspaceSegmentEditorSegmentForRepair = (
...
const applySegmentEditorSpeechWordsRange = (segment: WorkspaceSegmentEditorSegment) => {
...
const setWorkspaceSegmentEditorSegmentEndTime = (
  segment: WorkspaceSegmentEditorSegment,
  endTime: number,
) => {
...
const setWorkspaceSegmentEditorSegmentStartTime = (
  segment: WorkspaceSegmentEditorSegment,
  startTime: number,
) => {
...
const canRepairWorkspaceSegmentEditorSharedVoiceoverBoundary = (
  current: WorkspaceSegmentEditorSegment,
  next: WorkspaceSegmentEditorSegment,
) =>
  normalizePositiveProjectId(current.voiceoverAssetId ?? current.voiceover?.media_asset_id) === null &&
  normalizePositiveProjectId(next.voiceoverAssetId ?? next.voiceover?.media_asset_id) === null;

const repairWorkspaceSegmentEditorSpeechWordBoundaries = (
  segments: WorkspaceSegmentEditorSegment[],
): WorkspaceSegmentEditorSegment[] => {
  ...
  for (let index = 0; index < repairedSegments.length - 1; index += 1) {
    ...
    if (currentSpeechRange && nextSpeechRange) {
      const repairedBoundary = roundSegmentEditorTimelineSeconds(
        Math.max(current.endTime, next.startTime, currentSpeechRange.endTime, nextSpeechRange.startTime),
      );
...
const isSegmentEditorPreparingUpstreamError = (error: UpstreamHttpError) =>
...
const getSegmentEditorVoiceoverAssetDurationCacheKey = (assetId: number) =>
  `asset:${assetId}`;
...
const readCachedSegmentEditorVoiceoverAssetDuration = (assetId: number) => {
  const cachedEntry = segmentEditorVoiceoverDurationCache.get(
    getSegmentEditorVoiceoverAssetDurationCacheKey(assetId),
  );
  if (!cachedEntry) {
    return null;
  }
...
const writeCachedSegmentEditorVoiceoverAssetDuration = (
  assetId: number,
  durationSeconds: number,
) => {
...
const fetchWorkspaceVoiceoverAssetDuration = async (assetId: number, projectId: number) => {
  ...
  const response = await fetchUpstreamResponse(
    buildAdsflowUrl(`/api/media/${assetId}/download`),
    {
      headers: {
        "X-Admin-Token": env.adsflowAdminToken ?? "",
      },
...
const assertWorkspaceSegmentEditorStandaloneVoiceovers = (session: WorkspaceSegmentEditorSession) => {
  const projectVoiceKey = normalizeSegmentEditorVoiceKey(session.voiceType);
  const missingVoiceover = session.segments.some((segment) => {
    if (!normalizeText(segment.text)) {
      return false;
    }
    const segmentVoiceKey = normalizeSegmentEditorVoiceKey(segment.voiceType);
    const effectiveVoiceKey = segmentVoiceKey || projectVoiceKey || normalizeSegmentEditorVoiceKey(segment.voiceoverVoiceType);
    if (!effectiveVoiceKey || SEGMENT_EDITOR_DISABLED_VOICE_TYPES.has(effectiveVoiceKey)) {
      return false;
    }
    return normalizePositiveProjectId(segment.voiceoverAssetId ?? segment.voiceover?.media_asset_id) === null;
  });
...
const hasWorkspaceSegmentEditorLegacyAssetLocalSpeechTiming = (
  segment: WorkspaceSegmentEditorSegment,
) => {
  ...
  if (
    explicitAssetDuration !== null &&
    explicitAssetDuration > 0 &&
    declaredTimingValues.some(
      (value) => value > explicitAssetDuration + SEGMENT_EDITOR_SPEECH_TIMING_EPSILON_SECONDS,
    )
  ) {
    return false;
  }
  ...
  const assetDuration =
    normalizeNumber(segment.voiceSourceDuration) ??
...
export const normalizeWorkspaceSegmentEditorStandaloneSpeechTiming = (
  segment: WorkspaceSegmentEditorSegment,
  projectTtsAssetId: number | null,
): WorkspaceSegmentEditorSegment => {
  ...
  const sourceDuration =
    normalizeNumber(segment.voiceSourceDuration) ??
    normalizeNumber(segment.speechDuration);
  const normalizedSourceDuration =
    sourceDuration !== null && sourceDuration > 0
      ? roundSegmentEditorTimelineSeconds(sourceDuration)
      : null;
  const speechTimingCoordinateSpace = normalizeWorkspaceSegmentSpeechTimingCoordinateSpace(
    segment.speechTimingCoordinateSpace,
  );
  const voiceSourceCoordinateSpace = normalizeWorkspaceSegmentVoiceSourceCoordinateSpace(
    segment.voiceSourceCoordinateSpace,
  );
  const shouldConvertAssetLocalSpeech =
...
const enrichWorkspaceSegmentEditorSessionWithVoiceoverDurations = async (
  ...
  const normalizedSession = {
    ...
    segments: session.segments.map((segment) =>
      normalizeWorkspaceSegmentEditorStandaloneSpeechTiming(segment, session.ttsAssetId)),
  };
  ...
  const measuredVoiceoverAssetDurations = await Promise.all(
    voiceoverAssetSegmentsNeedingMeasurement.map(async (segment) => {
      const assetId = normalizePositiveProjectId(segment.voiceoverAssetId ?? segment.voiceover?.media_asset_id);
      if (assetId === null) {
  ...
  const durationBySegmentIndex = new Map(
    measuredVoiceoverAssetDurations.filter((entry): entry is readonly [number, number] => entry[1] !== null),
  );
    ...
    segments: normalizedSession.segments.map((segment) => {
      ...
      return {
        ...
        ...(preservePreciseSpeechTiming
          ? {
              voiceSourceCoordinateSpace: "asset_local" as const,
              voiceSourceDuration: durationSeconds,
...
const shouldLoadSourceSegmentEditorPayload = (
  ...
  return needsMusic ? sourceProjectId : null;
};
...
const loadSourceSegmentEditorPayload = async (
  ...
  try {
    ...
    console.warn("[segment-editor] Failed to load source segment editor music metadata", {
      error: error instanceof Error ? error.message : String(error),
      projectId,
      sourceProjectId,
    });
...
export const buildWorkspaceSegmentEditorSessionFromPayload = (
  ...
  const usesCurrentProjectTimeline =
    Array.isArray(generationSettings?.video_segments) &&
    generationSettings.video_segments.length > 0 &&
    authoritativeTimelineSegments === generationSettings.video_segments;
  const authoritativeTimelineSegmentIndexes = authoritativeTimelineSegments.map((segment, fallbackIndex) =>
  ...
  const authoritativeTimelineSegmentsByIndex = new Map(
    authoritativeTimelineSegments.map((segment, fallbackIndex) => [
      getSegmentEditorSegmentIndex(segment, fallbackIndex),
      segment,
    ] as const),
  );
  const payloadSegments = payload.segments ?? [];
  const payloadSegmentIndexes = payloadSegments.map((segment, fallbackIndex) =>
    getSegmentEditorSegmentIndex(segment, fallbackIndex));
  assertUniqueSegmentEditorIndexes(payloadSegmentIndexes, "редактора сегментов");
  const payloadSegmentsByIndex = new Map(
  ...
  if (usesCurrentProjectTimeline) {
    ...
    const hasPartialExplicitIndexes = (indexes: Array<number | null>) => {
      const explicitCount = indexes.filter((index): index is number => index !== null).length;
    ...
    const canUseLegacyPositionalIdentity = (
      hasAuthoritativeExplicitIndexes !== hasPayloadExplicitIndexes &&
      (hasAuthoritativeExplicitIndexes ? authoritativeExplicitIndexes : payloadExplicitIndexes)
        .every((index, slot) => index === slot)
    );
    if (
      hasPartialExplicitIndexes(authoritativeExplicitIndexes) ||
      ...
      throw new WorkspaceSegmentEditorError(
        "Актуальный таймлайн проекта и данные редактора используют неоднозначные индексы сегментов.",
        409,
      );
    }
    ...
    const hasMismatchedSegmentSet =
      authoritativeIndexSet.size !== payloadIndexSet.size ||
      [...authoritativeIndexSet].some((segmentIndex) => !payloadIndexSet.has(segmentIndex));
    if (hasMismatchedSegmentSet) {
      throw new WorkspaceSegmentEditorError(
        "Актуальный таймлайн проекта и данные редактора содержат разные сегменты. Обновите проект и повторите попытку.",
        409,
      );
    }
  }
  ...
  const segmentsToBuild = usesCurrentProjectTimeline
    ? authoritativeTimelineSegments.map((authoritativeSegment, fallbackIndex) => {
        ...
        if (!segment) {
          throw new WorkspaceSegmentEditorError(
            `Данные редактора для сегмента ${segmentIndex + 1} отсутствуют.`,
            409,
          );
        }
  ...
  const rawSegments = segmentsToBuild
    .map(({ authoritativeSegment, segment }) =>
...
export const buildWorkspaceSegmentEditorSegment = (
  ...
  projectSources?: {
    ...
    currentEntriesBySegmentIndex?: ReadonlyMap<number, AdsflowProjectMediaEntryPayload>;
    projectMediaAssets?: WorkspaceMediaAssetRef[];
    projectMediaLoaded?: boolean;
    projectMediaByAssetId: Map<number, WorkspaceMediaAssetRef>;
    originalEntries: AdsflowProjectMediaEntryPayload[];
    originalEntriesBySegmentIndex?: ReadonlyMap<number, AdsflowProjectMediaEntryPayload>;
  ...
  const explicitOriginalEntry =
    projectSources?.originalEntriesBySegmentIndex?.get(index) ??
...
const loadWorkspaceSegmentEditorSession = async (projectId: number): Promise<WorkspaceSegmentEditorSession> => {
  ...
  try {
    ...
    if (
      ...
      isSegmentEditorPreparingUpstreamError(error)
    ) {
      projectDetailsPayload = await withOptionalSegmentEditorContextTimeout(
        projectDetailsPromise,
        null,
    ...
    if (error instanceof UpstreamHttpError && error.statusCode === 409 && !payload) {
      throw new WorkspaceSegmentEditorError(normalizeText(error.message), 409);
    }
  ...
  payload = hydrateSegmentEditorPayloadWithInheritedMusic(payload, projectDetailsPayload, sourcePayload);

  const session = buildWorkspaceSegmentEditorSessionFromPayload(projectId, payload, {
    projectDetailsPayload,
    projectMediaEnvelope,
  });
  assertWorkspaceSegmentEditorStandaloneVoiceovers(session);
...
export async function getWorkspaceProjectSegmentVoiceoverProxyTarget(
  ...
  return {
    ...
    url: buildAdsflowUrl(`/api/projects/${session.projectId}/segments/${options.segmentIndex}/voiceover`),
  };
}
