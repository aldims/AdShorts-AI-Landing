import {
  type ProjectMediaEnvelope,
  resolveWorkspaceMediaAssetLifecycle,
  type WorkspaceMediaAssetRef,
} from "../shared/workspace-media-assets.js";
...
export type AdsflowMediaAssetPayload = {
...
export const buildWorkspaceMediaAssetRef = (value: Partial<AdsflowMediaAssetPayload> | null | undefined) => {
  ...
  return {
    ...
    status,
    storageKey,
  } satisfies WorkspaceMediaAssetRef;
};

export const mergeWorkspaceMediaAssetRefs = (
  primary: WorkspaceMediaAssetRef | null | undefined,
  fallback: WorkspaceMediaAssetRef | null | undefined,
...
export const fetchProjectMediaEnvelope = async (projectId: number): Promise<ProjectMediaEnvelope> => {
  ...
  const payload = await fetchAdsflowJson<AdsflowProjectMediaResponse>({
    context: {
  ...
  const assets = Array.isArray(payload?.assets)
    ? payload.assets
        ...
        .filter((item): item is WorkspaceMediaAssetRef => Boolean(item))
    : [];
