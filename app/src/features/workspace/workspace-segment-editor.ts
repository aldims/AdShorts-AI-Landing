...
export const WORKSPACE_SEGMENT_EDITOR_NEW_SEGMENT_DURATION_SECONDS = 2.4;
...
export const buildWorkspaceProjectMusicAudioProxyUrl = (
  ...
  if (normalizedVersion) {
    ...
    if (
      ...
      params.set("musicName", normalizedVersion);
    }
  }
...
export const getWorkspaceSegmentVoiceoverAudioPreviewSource = (options: {
  ...
  const canUseSegmentVoiceoverAudioProxy = canUseProjectVoiceoverAudio && hasProjectVoiceoverTiming;
  ...
  const audioUrl = isVoiceAudioStale
    ...
    : latestSceneVoiceoverAudioUrl ??
      ...
      (preferSegmentProxy ? segmentVoiceoverAudioUrl : null);
  const sourceKind =
    audioUrl === null
      ? null
...
export const getWorkspaceSegmentKnownVisualDurationSeconds = (segment: WorkspaceSegmentEditorDraftSegment) => {
  ...
  if (mediaAssetDuration !== null) {
    return mediaAssetDuration;
  }

  const durationExtensionSourceDurationSeconds = getWorkspaceSegmentStoredDurationExtensionSourceDurationSeconds(segment);
  if (durationExtensionSourceDurationSeconds !== null) {
    return durationExtensionSourceDurationSeconds;
  }
...
export const syncWorkspaceSegmentMeasuredVideoVisualDuration = (
  ...
  const storedDurationExtensionSourceDurationSeconds =
    getWorkspaceSegmentStoredDurationExtensionSourceDurationSeconds(segment);
  const canCorrectMeasuredVisualDuration =
    durationMode === "manual" &&
    durationSyncMode === "visual" &&
    manualDurationSeconds !== null &&
    storedDurationExtensionSourceDurationSeconds === null &&
    manualDurationSeconds + WORKSPACE_SEGMENT_EXTENSION_EPSILON_SECONDS < measuredVisualDuration;
  const hasManualTimelineOverride =
    durationMode === "manual" ||
    manualDurationSeconds !== null ||
    storedDurationExtensionSourceDurationSeconds !== null;
  if (hasManualTimelineOverride && !canCorrectMeasuredVisualDuration) {
    return segment;
  }
...
const syncWorkspaceSegmentGeneratedVideoDefaultVisualDuration = (
  ...
  if (
    getWorkspaceSegmentKnownVisualDurationSeconds(segment) !== null ||
  ...
  if (
    manualDurationSeconds === null ||
    manualDurationSeconds + WORKSPACE_SEGMENT_EXTENSION_EPSILON_SECONDS >= WORKSPACE_SEGMENT_AI_EXTENSION_STEP_SECONDS
  ) {
    return segment;
  }

  const duration = roundWorkspaceSegmentTimelineSeconds(WORKSPACE_SEGMENT_AI_EXTENSION_STEP_SECONDS);
  const startTime = getWorkspaceSegmentEditorDisplayStartTime(segment);
  return {
    ...segment,
    duration,
...
export const syncWorkspaceSegmentFreshVoiceoverTimelineDuration = (
  ...
  if (shouldAutoTrimWorkspaceSegmentVideoToVoiceover(currentVideoDurationSeconds, voiceoverDurationSeconds)) {
...
export const rebuildWorkspaceSegmentEditorDraftTimeline = (
  ...
  const syncedSegments = syncWorkspaceSegmentsEmbeddedVisualDurations(segments).map((segment) => {
    ...
    const segmentKeptManualTimelineOverride =
      normalizeWorkspaceSegmentDurationMode(segmentWithFreshVoiceoverTiming.durationMode) === "manual" ||
      normalizeWorkspaceSegmentManualDurationSeconds(segmentWithFreshVoiceoverTiming.manualDurationSeconds) !== null ||
      getWorkspaceSegmentStoredDurationExtensionSourceDurationSeconds(segmentWithFreshVoiceoverTiming) !== null;
    if (
      segmentHadManualTimelineOverride &&
      !segmentKeptManualTimelineOverride &&
      getWorkspaceSegmentTimelineVoiceoverDurationInfo(segmentWithFreshVoiceoverTiming, session, {
        allowEstimated: false,
      }) !== null
    ) {
      hasVoiceoverTimelineDurationReset = true;
    }
    if (hasWorkspaceSegmentStaleVisualDurationAfterVoiceoverSync(segmentWithFreshVoiceoverTiming, session)) {
      hasVoiceoverTimelineDurationReset = true;
    }
...
export const normalizeLegacyWorkspaceSegmentEditorDraftSession = (
  ...
  const segments = session.segments.map((segment) => {
    ...
    const hasAiPhotoChanges =
      ...
      normalizedDurationSyncMode !== normalizeWorkspaceSegmentDurationSyncMode(segment.durationSyncMode) ||
    ...
    if (
      ...
      return {
        ...
        customVideo: cloneStudioCustomVideoFile(segment.customVideo),
        durationSyncMode: normalizedDurationSyncMode,
